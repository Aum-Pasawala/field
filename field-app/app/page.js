...
const GRADE_COLORS = {
  "A+": "#00E5B8", "A": "#00E5B8", "A-": "#00D4A8",
  "B+": "#7CCA5C", "B": "#A8CC44", "B-": "#C4CC22",
  "C+": "#FFD60A", "C": "#FFB800", "C-": "#FF9900",
  "D+": "#FF6B2B", "D": "#FF4D2B", "D-": "#FF2952",
  "F": "#FF2952", "?": "#383854",
};
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
            ...
            <div style={{ textAlign: "center", padding: "0 16px" }}>
              ...
              <div style={{ fontSize: 12, color: game.status === "live" ? C.red : C.textMid, fontFamily: Fb, fontWeight: 700, letterSpacing: "1px", marginTop: 4 }}>
                {game.status === "live" ? "\u{1F534} " + game.period : game.status === "final" ? "FINAL" : game.time}
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
...
function AnalysisPanel({ analysis, loading, type }) {
    ...
    return (
      <div style={{
        ...
        <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ height: 14, background: C.accent + "12", borderRadius: 4, width: "90%", animation: "pulse 1.5s infinite" }} />
          <div style={{ height: 14, background: C.accent + "08", borderRadius: 4, width: "70%", animation: "pulse 1.5s infinite", animationDelay: "0.2s" }} />
          <div style={{ height: 14, background: C.accent + "06", borderRadius: 4, width: "80%", animation: "pulse 1.5s infinite", animationDelay: "0.4s" }} />
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{
      marginTop: 16,
      background: "linear-gradient(135deg, " + C.accent + "0A, " + C.accent + "04)",
      borderRadius: 12,
      border: "1px solid " + C.accent + "25",
      overflow: "hidden",
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 800, color: C.accentBright, fontFamily: Fb, letterSpacing: "2px", textTransform: "uppercase" }}>
            AI Analysis
          </span>
        </div>
        {showGrade && (
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{
              fontSize: 20, fontWeight: 900, fontFamily: Fd, color: gradeColor,
      ...
      <div style={{ padding: "16px 20px", display: "flex", flexDirection: "column", gap: 14 }}>
        {analysis.summary && (
          <div style={{ fontSize: 15, color: C.text, fontFamily: Fb, fontWeight: 500, lineHeight: 1.55 }}>
            {analysis.summary}
          </div>
        )}

        {showGrade && analysis.gradeReason && (
          <div style={{
            padding: "10px 14px",
            background: gradeColor + "10",
            borderRadius: 8,
            borderLeft: "3px solid " + gradeColor + "60",
          }}>
            <span style={{ fontSize: 13, color: gradeColor, fontFamily: Fb, fontWeight: 600 }}>
        ...
        {analysis.impact && (
          <div>
            <div style={{ fontSize: 11, fontWeight: 800, color: C.orange, fontFamily: Fb, letterSpacing: "1.5px", marginBottom: 6, textTransform: "uppercase" }}>
              {type && ["trade","injury","signing","roster","news","award","storyline"].includes(type) ? "Team & League Impact" : "World Impact"}
            </div>
            <div style={{ fontSize: 14, color: C.textMid, fontFamily: Fb, lineHeight: 1.55 }}>
              {analysis.impact}
            </div>
          </div>
        )}
        ...
        {analysis.context && (
          <div>
            <div style={{ fontSize: 11, fontWeight: 800, color: C.cyan, fontFamily: Fb, letterSpacing: "1.5px", marginBottom: 6, textTransform: "uppercase" }}>
              Historical Context
            </div>
            <div style={{ fontSize: 14, color: C.textMid, fontFamily: Fb, lineHeight: 1.55 }}>
              {analysis.context}
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 12, marginBottom: 26 }}>
              <Pill active={section === "scores"} onClick={() => setSection("scores")}>
                {"\u{1F3AE}"} Scores {liveCount > 0 && <span style={{ marginLeft: 8, background: C.red, color: "#fff", borderRadius: 10, padding: "2px 7px", fontSize: 11, fontWeight: 800 }}>{liveCount} LIVE</span>}
              </Pill>
