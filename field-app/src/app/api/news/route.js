...
// HARD DISCARD — any match = article gone
const TRASH = [
...
// Must match a known category — if it doesn't fit, DISCARD (no geo fallback for garbage)
const CATS = {
...
function categorize(title = "", desc = "") {
  ...
  return null; // null = no match = discard
}
  ...
  try {
    ...
    const articles = data.articles
      .filter(a => {
        ...
        if (isTrash(h)) return false;          // hard discard
        if (!isReal(h)) return false;          // must be real news
        const cat = categorize(h, a.description || "");
        return cat !== null;                   // must fit a category
      })
      .slice(0, 15)
      .map((a, i) => {
    ...
    // If not enough real articles, fall back to curated list
