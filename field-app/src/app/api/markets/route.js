...
export async function GET() {
  try {
    ...
    const url  = `https://query1.finance.yahoo.com/v7/finance/quote?symbols=${syms}&fields=regularMarketPrice,regularMarketChange,regularMarketChangePercent,regularMarketPreviousClose,fiftyTwoWeekHigh,fiftyTwoWeekLow`;
    ...
    const tickers = SYMBOLS.map(s => {
      ...
      if (!q) return { symbol: s.label, value: "--", change: "--", up: true, type: s.type, changeAbs: "--" };

      const price     = q.regularMarketPrice;
      const changePct = q.regularMarketChangePercent || 0;
      const changeAbs = q.regularMarketChange || 0;
      ...
      else value = price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });

      return {
        symbol:    s.label,
        value,
        change:    `${up ? "+" : ""}${changePct.toFixed(2)}%`,
        changeAbs: `${up ? "+" : ""}${changeAbs.toFixed(2)}`,
        up,
        type:      s.type,
    ...
    return Response.json({ tickers: fallbackTickers() });
  }
}
